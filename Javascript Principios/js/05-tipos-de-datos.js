'use strict'

// Operadores
var numero1 = 7;
var numero2 = 12;
var operacion = numero1 * numero2;

alert("El resultado de la operacion es: " + operacion);

// Tipos de datos
var numero_entero = 44;
var cadena_texto = "Hola 'que' tal?";
var verdadero_o_falso = false;

var numero_falso = "33.4";

console.log(Number(numero_falso) + 7);	// Number() convierte a nro flotante
console.log(parseInt(numero_falso) + 7); // parseInt convierte a int
console.log(parseFloat(numero_falso) + 7); // parseInt convierte a float
console.log(numero_entero + 7);
console.log(numero_entero + " Texto concatenado");
console.log(String(numero_entero) + 4);

console.log("");

console.log(typeof numero_entero);
console.log(typeof cadena_texto);
console.log(typeof verdadero_o_falso);
console.log(typeof numero_falso);